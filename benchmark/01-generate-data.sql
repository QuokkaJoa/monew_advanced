...
--
-- UUID 는 전부 계산해서 만든다. 몇 번을 돌려도 같은 값이 나온다.
-- 삭제 처리는 19 번째 댓글마다 한다.
-- 기사별로 고르게 퍼졌는지는 맨 아래 확인 쿼리를 보고 판단한다.
--
-- 실행:
...
-- 만든 데이터가 의도한 모양인지 확인한다.
-- 삭제율 최소와 최대가 비슷하면 삭제가 고르게 퍼진 것이다.
-- 한쪽이 0 에 가깝고 다른 쪽이 크게 튀면 잘못 만든 것이다.

SELECT
    count(*)                                                        AS "대상 기사 댓글",
    count(*) FILTER (WHERE active)                                  AS "활성",
    round(100.0 * count(*) FILTER (WHERE NOT active) / count(*), 1) AS "삭제율"
FROM comments_managements
WHERE news_article_id = 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee';

WITH per_article AS (
    SELECT count(*) AS total,
           round(100.0 * count(*) FILTER (WHERE NOT active) / count(*), 1) AS del_rate
    FROM comments_managements
    GROUP BY news_article_id
)
SELECT
    (SELECT count(*) FROM comments_managements) AS "댓글 전체",
    count(*)                                    AS "기사 수",
    min(total)                                  AS "기사당 최소",
    max(total)                                  AS "기사당 최대",
    min(del_rate)                               AS "삭제율 최소",
    max(del_rate)                               AS "삭제율 최대"
FROM per_article;
